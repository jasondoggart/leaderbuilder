class FetchUsers
  ...
  def get_people
    ...
    people_url = URI("https://api.planningcenteronline.com/people/v2/people?where[last_name]=" + @last_name)
    ...
    request.basic_auth ENV['PLANNING_CENTER_APPLICATION_ID'], ENV['PLANNING_CENTER_SECRET']
    ...
    people['data'].each do |person|
      ...
      email_url = URI("https://api.planningcenteronline.com/people/v2/people/" + person_id + "/emails")
      email_http = Net::HTTP.new(email_url.host, email_url.port)
      email_http.use_ssl = true
      email_request = Net::HTTP::Get.new(email_url)
      email_request.basic_auth ENV['PLANNING_CENTER_APPLICATION_ID'], ENV['PLANNING_CENTER_SECRET']
      ...
      if email['meta']['total_count'] > 0
        ...
        people_hash[person_id.to_sym][:email] = 'none'
      end
